...
def cross_testing(estimator, X_train, X_test, Y_train, Y_test, cv, model_params=None):
    ...
    for X_test_index, Y_test_index in zip(cv.split(X_test), cv.split(Y_test)):
        ...
        accuracies.append(accuracy_score(clf.predict(X_test[X_test_index[0]]), Y_test[Y_test_index[0]]))
    return accuracies
